const C=document.getElementById('game'); const ctx=C.getContext('2d',{alpha:false});
let DPR=Math.max(1,window.devicePixelRatio||1);
function resize(){ DPR=Math.max(1,window.devicePixelRatio||1); C.width=Math.floor(window.innerWidth*DPR); C.height=Math.floor(window.innerHeight*DPR); C.style.width='100vw'; C.style.height='100vh'; ctx.setTransform(DPR,0,0,DPR,0,0);} 
...
function initAudio(){ if(audioReady)return; ac=new(window.AudioContext||window.webkitAudioContext)(); master=ac.createGain(); master.gain.value=.6; dist=ac.createWaveShaper(); dist.curve=new Float32Array(44100).map((_,i)=>{const x=i/44100*2-1;const k=50;return (1+k)*x/(1+k*Math.abs(x))}); gain=ac.createGain(); gain.gain.value=.9; gain.connect(dist); dist.connect(master); master.connect(ac.destination); audioReady=true;}
function metalChug(){ if(!audioReady)return; const o=ac.createOscillator(),g=ac.createGain(); o.type='sawtooth'; o.frequency.setValueAtTime(55,ac.currentTime); g.gain.setValueAtTime(.0001,ac.currentTime); g.gain.exponentialRampToValueAtTime(.5,ac.currentTime+.02); g.gain.exponentialRampToValueAtTime(.001,ac.currentTime+.25); o.connect(g); g.connect(gain); o.start(); o.stop(ac.currentTime+.26); }
...
const state={started:false,dead:false,px:200,py:200,vx:0,vy:0,dir:1,hp:100,score:0,wave:1,rage:0,fury:false,furyTimer:0,zombies:[],bullets:[],particles:[]};

const hpEl=document.getElementById('hp'),scoreEl=document.getElementById('score'),waveEl=document.getElementById('wave'),rageEl=document.getElementById('rage');
const overlay=document.getElementById('overlay');
document.getElementById('startBtn').addEventListener('click',()=>{initAudio();startGame();});
document.getElementById('btnSlash').addEventListener('click',()=>attackSlash());
document.getElementById('btnShoot').addEventListener('click',()=>attackShoot());
document.getElementById('btnRage').addEventListener('click',()=>activateRage());

const stick={el:document.getElementById('stickL'),nub:document.querySelector('#stickL .nub'),dx:0,dy:0,r:60};
...
stick.el.addEventListener('touchstart',e=>{stickEvt(e);},{passive:true});
stick.el.addEventListener('touchmove',stickEvt,{passive:true});
stick.el.addEventListener('touchend',()=>{stick.dx=stick.dy=0;stick.nub.style.transform='translate(-50%,-50%)';});

function startGame(){overlay.style.display='none';Object.assign(state,{started:true,dead:false,px:window.innerWidth/2,py:window.innerHeight/2,vx:0,vy:0,hp:100,score:0,wave:1,rage:0,fury:false,furyTimer:0,zombies:[],bullets:[],particles:[]});for(let i=0;i<8;i++)spawnZombie();loop();}

function spawnZombie(speedMul=1){const x=R.next()<.5?-40:C.width/DPR+40;const y=80+R.next()*(C.height/DPR-160);state.zombies.push({x,y,hp:20+state.wave*4,spd:(.4+.4*R.next())*speedMul});}
...
function loop(t=0){if(!state.started)return;raf=requestAnimationFrame(loop);const dt=Math.min(33,t-last||16)/16;last=t;
  const speed=state.fury?4:2.6;state.vx=stick.dx*speed;state.vy=stick.dy*speed;state.px=Math.max(40,Math.min(window.innerWidth-40,state.px+state.vx));state.py=Math.max(60,Math.min(window.innerHeight-60,state.py+state.vy));if(Math.abs(state.vx)>0.02)state.dir=state.vx>0?1:-1;
  state.bullets.forEach(b=>{b.x+=b.vx;b.y+=b.vy;b.life--;});state.bullets=state.bullets.filter(b=>b.life>0);
  state.zombies.forEach(z=>{const dx=state.px-z.x,dy=state.py-z.y;const d=Math.hypot(dx,dy)+0.001;z.x+=(dx/d)*z.spd*(state.fury?.6:1);z.y+=(dy/d)*z.spd*(state.fury?.6:1);
    state.bullets.forEach(b=>{if(Math.hypot(b.x-z.x,b.y-z.y)<14){z.hp-=18*(state.fury?1.6:1);b.life=0;spawnParticles(z.x,z.y,'#f55');}});
    if(d<24&&!state.dead){state.hp-=0.15*(state.fury?0.3:1);if(Math.random()<.02)boomKick();if(state.hp<=0){state.hp=0;state.dead=true;gameOver();}}
  });
...
function render(){const g=ctx,w=C.width/DPR,h=C.height/DPR;g.fillStyle='#0a0a0a';g.fillRect(0,0,w,h);
  ...
  g.fillStyle='#111';for(let i=0;i<Math.ceil(w/90)+2;i++){const x=(i*90+(i%2?40:0))%w;g.fillRect(x,h-80-(i%3)*18,30,80+(i%3)*6);}
...
function drawHero(g,x,y,dir,fury){g.save();g.translate(x,y);g.fillStyle='#202020';g.fillRect(-18,22,36,14);g.strokeStyle='#333';g.lineWidth=3;g.beginPath();g.moveTo(-8,6);g.lineTo(-8,22);g.moveTo(8,6);g.lineTo(8,22);g.stroke();g.fillStyle='#1a1a1a';g.beginPath();g.moveTo(-20,0);g.lineTo(20,0);g.lineTo(18,8);g.lineTo(-18,10);g.closePath();g.fill();g.fillStyle='#101010';g.fillRect(-12,-18,24,18);g.fillStyle='#d0b09a';g.beginPath();g.arc(0,-26,10,0,6.28);g.fill();g.fillStyle=fury?'#ff9a3d':'#caa86a';g.beginPath();g.ellipse(0,-28,14,10,0,0,6.28);g.fill();g.fillStyle=fury?'#fff':'#f55';g.fillRect(-4,-28,3,2);g.fillRect(1,-28,3,2);g.save();g.translate(dir>0?16:-16,-10);g.rotate(dir>0?-0.1:Math.PI+0.1);g.fillStyle='#9a9a9a';g.fillRect(0,-3,36,6);g.fillStyle='#3a3a3a';g.fillRect(-6,-4,12,8);g.restore();g.fillStyle='#382e2e';g.fillRect(-6,-12,12,26);if(fury){g.strokeStyle='rgba(255,120,0,.8)';g.lineWidth=2;g.beginPath();g.arc(0,-10,28,0,6.28);g.stroke();}g.restore();}
